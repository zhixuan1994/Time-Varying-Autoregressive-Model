...
class TV_VAR_GAM:
    ...
    def TV_VAR_lagged(self, linear_max_order=3):
        ...
        len_basis_func = t_train_processed.shape[1]
    ...
    def GAM_predict(self, TV_VAR_x_list, predict_data, t_list_r, t_h, predict_step):
        if len(TV_VAR_x_list) < self.series_data.shape[1]:
...
class TV_VAR_OKS:
    ...
    def one_side_KS_predict(self, Y, beta_t_star_ma, predict_step):
        Y_pred_data = Y
...
class TV_VAR_trad:
    def __init__(self, lag_r: int, series: Union[List, np.ndarray, pd.Series], 
        ...
        self.series_data = np.array(series)
        self.t_list = t_list
        self.VAR_shape_0 = self.series_data[1]

    def estimate_var_coefficients(self):
        X = np.hstack([self.series_data[self.lag_r - i - 1:self.series_data.shape[0] - i - 1] for i in range(self.lag_r)])
        Y_target = self.series_data[self.lag_r:]
        X_with_intercept = np.hstack([np.ones((X.shape[0], 1)), X])
        B_hat = np.linalg.inv(X_with_intercept.T @ X_with_intercept) @ (X_with_intercept.T @ Y_target)
        intercept = B_hat[0]
        A_flat = B_hat[1:].T
        A_list = [A_flat[:, i*self.series_data.shape[1]:(i+1)*self.series_data.shape[1]] for i in range(self.lag_r)]
        return intercept, A_list

    def forecast_var_p(self, A_list, intercept, predict_step):
        p = len(A_list)
        preds = []
        Y_current = self.series_data[-p:].tolist()
